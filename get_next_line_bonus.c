...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/16 16:20:14 by zaabou            #+#    #+#             */
/*   Updated: 2021/12/16 20:56:47 by zaabou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	rest[fd] = ft_read_fd(rest[fd], fd);
	line_ = ft_line_(rest[fd]);
	rest[fd] = ft_next_line(rest[fd]);
	if (!rest[fd] && ft_end(line_))
